...
export function ProviderDetail({ id }: { id: string }) {
  ...
  const healthyKeys = health?.keys.filter((k, i) => k.healthy && !(provider.disabled_keys?.includes(i) ?? false)).length ?? 0;
  ...
  return (
    <div>
        ...
        <div className="px-6 py-5">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-[30px] font-bold tracking-tight heading-gradient heading-accent">{provider.name || provider.id}</h1>
          ...
          {editingConn && (
            <div className="mt-4 space-y-3 rounded-brand card p-4">
      ...
      {/* ─── Stat strip ─── */}
      <div className="mb-4 overflow-hidden rounded-brand-lg card">
        <div className="grid grid-cols-2 sm:grid-cols-4">
          <div className="border-b border-border-subtle px-5 py-4 sm:border-b-0 sm:border-r">
            <div className="text-[11px] font-medium uppercase tracking-wider text-text-subtle">Keys</div>
            <div className="mt-1 tnum text-[28px] font-bold tracking-tight text-text">{keys.length}</div>
          </div>
          <div className="border-b border-border-subtle px-5 py-4 sm:border-b-0 sm:border-r">
            <div className="text-[11px] font-medium uppercase tracking-wider text-text-subtle">Models</div>
            <div className="mt-1 tnum text-[28px] font-bold tracking-tight text-text">{provider.models.length}</div>
          </div>
          <div className="border-b border-border-subtle px-5 py-4 sm:border-b-0 sm:border-r">
            <div className="text-[11px] font-medium uppercase tracking-wider text-text-subtle">Strategy</div>
            <div className="mt-1 text-[15px] font-semibold text-text">
              {provider.strategy === "round-robin" ? `Round-Robin${provider.sticky ? ` · ${provider.sticky}` : ""}` : "Single"}
            </div>
          </div>
          <div className="px-5 py-4">
            <div className="text-[11px] font-medium uppercase tracking-wider text-text-subtle">Health</div>
            <div className="mt-1 tnum text-[28px] font-bold tracking-tight text-text">
              {health ? `${healthyKeys}/${health.keys.length}` : "—"}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`space-y-4 transition-opacity ${provider.disabled ? "opacity-50" : ""}`}>
        {/* ─── Keys ─── */}
        <div className="card rounded-brand-lg">
          <div className="flex items-center justify-between gap-3 border-b border-border-subtle px-5 py-3">
            <div className="min-w-0">
              <div className="truncate text-[14px] font-semibold tracking-tight text-text">Keys</div>
              <div className="truncate text-[12px] text-text-muted">{keys.length} configured</div>
            </div>
            <div className="flex items-center gap-3">
              {keys.length > 1 && (
          ...
          <div className="p-4">
            ...
            {bulkOpen && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setBulkOpen(false)}>
                <div className="w-full max-w-lg rounded-brand glass-strong modal-card p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="card rounded-brand-lg">
          <div className="flex items-center justify-between gap-3 border-b border-border-subtle px-5 py-3">
            <div className="min-w-0">
              <div className="truncate text-[14px] font-semibold tracking-tight text-text">Models served</div>
              <div className="truncate text-[12px] text-text-muted">{provider.models.length} in catalog</div>
            </div>
