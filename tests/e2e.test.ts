...
describe("E2E — admin surface over real HTTP", () => {
  ...
  it("reveals a raw provider/gateway key only with admin auth", async () => {
    ...
    const cfg = (await (await fetch(gwUrl("/admin/config"), { headers: admin })).json()) as {
      ...
      server: { api_keys: string[]; key_models?: Record<string, string[]>; key_rpm?: Record<string, number> };
    };
    const masked = cfg.providers.find((p) => p.id === "oa-ok")!.api_key!;
    expect(masked).not.toBe("sk-oa");

    // key_models + key_rpm are re-keyed to the masked form (raw key never leaks)
    expect(cfg.server.key_models).toBeDefined();
    ...
    expect(cfg.server.key_rpm!["gw-key"]).toBeUndefined();
...
describe("E2E — scoped budget admin endpoints", () => {
  ...
  it("PUT adds a global budget, GET returns it, DELETE clears it", async () => {
    ...
    res = await fetch(gwUrl("/admin/budgets"), {
      method: "PUT",
      headers: admin,
      body: JSON.stringify({ scope: { type: "global" }, unit: "usd", limit: 50, window: "monthly" }),
    });
    expect(res.status).toBe(200);
  ...
  it("rejects a budget for an unknown provider scope with 400", async () => {
    const res = await fetch(gwUrl("/admin/budgets"), {
      method: "PUT",
      headers: admin,
